...
def convert_data_to_Labeltxt(df, _FolderImagesName_path, _ImageName_Column = "Image_name", _PositionBBoxName_Column = "Image Box" , _OCRName_Column = "SinoNom OCR"):
    # Grouping the data by the image name
    ...
    for page_id, group in grouped:
        ...
        for _, row in group.iterrows():
            points = sort_box(ast.literal_eval(row[f'{_PositionBBoxName_Column}'])) 
...
def main():
    ...
    result_file_path = 'MidTerm/output_csv/TTVH6_full_normalized.xlsx'

    # Chỉnh thành tên folder chứa thư mục ảnh của bạn
    folder_images_path = "MidTerm/filtered_data"
    ...
    _PositionBBoxName_Column = "ImageBox"
